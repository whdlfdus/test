...
# --- 페이지 설정 ---
st.set_page_config(layout="wide", page_title="데이터 분석 도구 v2.15") # 버전 업데이트
...
def init_session_state():
    defaults = {
        ...
        # 정제 설정
        'mv_settings': {'selected_cols': [], 'method': "결측치가 있는 행 전체 제거", 'specific_value': ""},
        'ot_settings': {'selected_cols': [], 'detection_method': "IQR 방식", 'iqr_multiplier': 1.5, 'zscore_threshold': 3.0, 'treatment_method': "결측치로 처리 (NaN으로 대체)"},
        'dd_settings': {'subset_cols': [], 'keep_method': "첫 번째 행 유지"},
        # 변환 설정
        'filter_rules': [], 'filter_conjunction': 'AND',
        'sort_rules': [],
        # 구조 변경 설정
        'pivot_settings': {'index_cols': [], 'columns_col': None, 'values_cols': [], 'agg_func': 'mean'},
        'unpivot_settings': {'id_vars': [], 'value_vars': [], 'var_name': 'variable', 'value_name': 'value'},
        # 파생 변수 (기존 수식 기반)
        ...
        'adv_builder_var_type': 'conditional', # 'conditional', 'window', 'mapping'
        ...
        'adv_builder_mapping_source_col': '', # 조건부 그룹핑용
        'adv_builder_mapping_rules': [], # 조건부 그룹핑용 (각 규칙: {'id': uuid, 'values_to_map': "값1,값2", 'new_group_name': "그룹명"})
        'adv_builder_mapping_default_group': '', # 조건부 그룹핑용
        # BigQuery
    ...
    if not st.session_state.adv_builder_conditional_rules: # 조건부 규칙 기본값
    ...
    if not st.session_state.adv_builder_mapping_rules: # 조건부 그룹핑 규칙 기본값
        st.session_state.adv_builder_mapping_rules = [{'id': str(uuid.uuid4()), 'values_to_map': '', 'new_group_name': ''}]
...
ADV_DERIVED_VAR_TYPES = ['conditional', 'window', 'mapping'] # 고급 파생 변수 유형 추가
...
def apply_advanced_derived_variables(df_input):
    ...
    for var_name, definition in st.session_state.advanced_derived_definitions.items():
        try:
            ...
            elif definition['type'] == 'mapping': # 조건부 그룹핑 (매핑) 적용 로직
                map_conf = definition['config']
                source_col = map_conf['source_col']
                mapping_rules = map_conf['rules']
                default_group = map_conf.get('default_group', None) # 기본값 처리

                if source_col not in current_df.columns:
                    st.warning(f"조건부 그룹핑 '{var_name}': 원본 열 '{source_col}'이 데이터에 없어 건너<0xEB><0><0x8F>니다.")
                    current_df[var_name] = pd.NA
                    continue
                
                # 초기값: 기본 그룹명 또는 원래 값
                if default_group is not None and default_group.strip() != "":
                    current_df[var_name] = default_group
                else: # 기본 그룹명이 없으면 원래 값을 유지
                    current_df[var_name] = current_df[source_col].astype(str) # 문자열로 변환하여 비교 준비

                # 매핑 규칙 적용 (np.select와 유사하게 또는 .loc 사용)
                # 뒤의 규칙이 앞의 규칙을 덮어쓸 수 있도록 역순으로 적용하거나, np.select 방식 사용
                conditions_map = []
                choices_map = []

                for rule in reversed(mapping_rules): # 우선순위를 위해 역순으로 처리 가능성 고려 (또는 np.select)
                    values_to_map_str = rule.get('values_to_map', '')
                    if not values_to_map_str.strip(): continue # 매핑할 값이 없으면 건너뛰기

                    # 쉼표로 구분된 값을 리스트로 변환 (공백 제거)
                    values_list = [v.strip() for v in values_to_map_str.split(',') if v.strip()]
                    if not values_list: continue

                    # 원본 열의 값이 매핑 대상 값들 중 하나에 해당하는지 확인
                    # 문자열로 변환하여 비교 (대소문자 구분 안 함 옵션 추가 가능)
                    condition = current_df[source_col].astype(str).isin(values_list)
                    conditions_map.append(condition)
                    choices_map.append(rule['new_group_name'])
                
                if conditions_map: # 적용할 매핑 규칙이 있다면
                    # np.select는 첫 번째 True 조건에 해당하는 choice를 선택
                    # default_value는 모든 조건이 False일 때 사용될 값
                    # 여기서는 이미 current_df[var_name]에 기본값이 설정되어 있으므로,
                    # .loc를 사용하여 조건에 맞는 행들만 업데이트하는 것이 더 직관적일 수 있음.
                    
                    # .loc를 사용한 방식 (규칙 순서대로 덮어쓰기)
                    # 초기값은 이미 설정됨 (default_group 또는 원본값)
                    for rule in mapping_rules: # 정의된 순서대로
                        values_to_map_str = rule.get('values_to_map', '')
                        if not values_to_map_str.strip(): continue
                        values_list = [v.strip() for v in values_to_map_str.split(',') if v.strip()]
                        if not values_list: continue
                        
                        condition = current_df[source_col].astype(str).isin(values_list)
                        current_df.loc[condition, var_name] = rule['new_group_name']
...
def _reset_dependent_states(all_cols, num_cols):
    ...
    keys_to_clear_or_default = [
        'mv_settings', 'ot_settings', 'dd_settings', # 정제 설정을 dict로 변경
        'filter_rules', 'sort_rules',
        'pivot_settings', 'unpivot_settings', # 구조 변경 설정을 dict로 변경
        'derived_var_name', 'derived_var_formula',
        'advanced_derived_definitions',
    ...
    for key in keys_to_clear_or_default:
        if key.endswith('_rules') or key == 'advanced_derived_definitions' or key.endswith('_settings'):
            if key == 'mv_settings': st.session_state[key] = {'selected_cols': [], 'method': "결측치가 있는 행 전체 제거", 'specific_value': ""}
            elif key == 'ot_settings': st.session_state[key] = {'selected_cols': [], 'detection_method': "IQR 방식", 'iqr_multiplier': 1.5, 'zscore_threshold': 3.0, 'treatment_method': "결측치로 처리 (NaN으로 대체)"}
            elif key == 'dd_settings': st.session_state[key] = {'subset_cols': [], 'keep_method': "첫 번째 행 유지"}
            elif key == 'pivot_settings': st.session_state[key] = {'index_cols': [], 'columns_col': None, 'values_cols': [], 'agg_func': 'mean'}
            elif key == 'unpivot_settings': st.session_state[key] = {'id_vars': [], 'value_vars': [], 'var_name': 'variable', 'value_name': 'value'}
            else: st.session_state[key] = [] if key != 'advanced_derived_definitions' else {} # filter_rules, sort_rules 등
    ...
    st.session_state.adv_builder_mapping_source_col = first_col_default_adv
    st.session_state.adv_builder_mapping_rules = [{'id': str(uuid.uuid4()), 'values_to_map': '', 'new_group_name': ''}]
    st.session_state.adv_builder_mapping_default_group = ''
...
# --- 데이터 처리 함수들 (apply_all_processing_steps 내부에서 호출될 함수들) ---
def _apply_missing_value_treatment(df, settings):
    if df is None or df.empty or not settings or not settings.get('selected_cols'):
        return df
    df_processed = df.copy()
    method = settings.get('method')
    specific_value = settings.get('specific_value')
    cols_to_process = [col for col in settings['selected_cols'] if col in df_processed.columns]

    if not cols_to_process: return df_processed

    if method == "결측치가 있는 행 전체 제거":
        df_processed.dropna(subset=cols_to_process, inplace=True)
    elif method == "특정 값으로 채우기":
        for col in cols_to_process:
            # 데이터 타입에 맞게 변환 시도
            try:
                if pd.api.types.is_numeric_dtype(df_processed[col]):
                    fill_val = pd.to_numeric(specific_value)
                elif pd.api.types.is_datetime64_any_dtype(df_processed[col]):
                    fill_val = pd.to_datetime(specific_value)
                else:
                    fill_val = specific_value
                df_processed[col].fillna(fill_val, inplace=True)
            except ValueError:
                st.warning(f"'{col}' 열의 결측치를 '{specific_value}'(으)로 채우는 중 타입 변환 오류 발생. 문자열로 처리합니다.")
                df_processed[col].fillna(str(specific_value), inplace=True)
    # (TODO: 나머지 결측치 처리 방법 구현)
    return df_processed

# (TODO: _apply_outlier_treatment, _apply_duplicate_removal, _apply_filters, _apply_sorts, _apply_pivot, _apply_unpivot 함수 구현 필요)
...
def apply_all_processing_steps():
    ...
    # 1. 데이터 정제
    current_df = _apply_missing_value_treatment(current_df, st.session_state.mv_settings)
    # current_df = _apply_outlier_treatment(current_df, st.session_state.ot_settings)
    # current_df = _apply_duplicate_removal(current_df, st.session_state.dd_settings)
    
    # 2. 데이터 변환
    # current_df = _apply_filters(current_df, st.session_state.filter_rules, st.session_state.filter_conjunction)
    # current_df = _apply_sorts(current_df, st.session_state.sort_rules)
    
    # 3. 데이터 구조 변경
    # if st.session_state.pivot_settings.get('index_cols') and st.session_state.pivot_settings.get('columns_col') and st.session_state.pivot_settings.get('values_cols'):
    #     current_df = _apply_pivot(current_df, st.session_state.pivot_settings)
    # elif st.session_state.unpivot_settings.get('id_vars'): # 언피벗은 id_vars만 있어도 가능
    #     current_df = _apply_unpivot(current_df, st.session_state.unpivot_settings)
...
with st.sidebar:
    ...
    if upload_method == "CSV 파일 업로드":
        uploaded_file = st.file_uploader("CSV 파일 선택", type="csv", key="file_uploader_v2_15") 
        if uploaded_file:
            if st.button("CSV 데이터 로드/업데이트", key="load_csv_button_v2_15", use_container_width=True): 
        ...
        if st.button("BigQuery 데이터 로드", key="load_bq_button_v2_15", use_container_width=True): 
    ...
    if df is not None and st.session_state.data_loaded_success:
        ...
        with st.expander("차트 옵션 보기/숨기기", expanded=True):
            ...
            if not is_pie_chart and not is_distribution_chart and not is_relationship_chart and not is_radar_chart and not is_heatmap_chart: 
                ...
                st.session_state.x_axis = st.selectbox("X축", headers if headers else ["선택 불가"], 
                                                       ...
                                                       disabled=not headers, key="x_axis_select_v2_15_common")
                ...
                st.session_state.group_by_col = st.selectbox("그룹화 기준 열 (선택)", group_by_options, 
                                                             ...
                                                             key="group_by_select_v2_15_common")
                
                st.session_state.agg_method = st.selectbox("집계 방식", ['Sum', 'Mean', 'Median'], index=['Sum', 'Mean', 'Median'].index(st.session_state.agg_method), key="agg_method_select_v2_15_common")
                ...
                if st.session_state.group_by_col != "None": 
                    ...
                    st.session_state.y_axis_single = st.selectbox("측정값 (기본 Y축)", available_measure_cols if available_measure_cols else ["선택 불가"], 
                                                                 ...
                                                                 disabled=not available_measure_cols, key="y_single_grouped_select_v2_15")
                else: 
                    if chart_type in ['선 (Line)', '누적 영역 (Stacked Area)', '누적 막대 (Stacked Bar)']: 
                        ...
                        st.session_state.y_axis_multiple = st.multiselect("기본 Y축 (다중 가능)", available_y_multi, default=current_y_multi, 
                                                                          disabled=not available_y_multi, key="y_multi_select_v2_15")
                        ...
                        st.session_state.y_axis_single = st.selectbox("기본 Y축", available_y_single_bar if available_y_single_bar else ["선택 불가"], 
                                                                     ...
                                                                     disabled=not available_y_single_bar, key="y_single_bar_select_v2_15")
                ...
                if not is_stacked_chart: 
                    ...
                    st.session_state.y_axis_secondary = st.selectbox("보조 Y축 (선택)", secondary_y_options, index=secondary_y_options.index(current_y_secondary), 
                                                                     disabled=not secondary_y_options[1:], key="y_secondary_select_v2_15")
                ...
                if chart_type == '히스토그램 (Histogram)':
                    ...
                    st.session_state.y_axis_multiple = st.multiselect("값 열 선택 (하나 이상, 숫자형)", numeric_headers, default=default_hist_val, disabled=not numeric_headers, key="hist_value_cols_v2_15")
                    ...
                    st.session_state.group_by_col = st.selectbox("색상 구분 열 (선택)", hist_color_options, index=hist_color_options.index(current_hist_color), 
                                                                 disabled=not hist_color_options[1:], key="hist_color_col_v2_15")
                    st.session_state.hist_bins = st.number_input("구간(Bin) 개수 (선택)", min_value=1, value=st.session_state.hist_bins if st.session_state.hist_bins and st.session_state.hist_bins > 0 else 20, step=1, key="hist_bins_v2_15")
                    ...
                    st.session_state.y_axis_multiple = st.multiselect("Y축 값 열 선택 (하나 이상, 숫자형)", numeric_headers, default=default_box_y, disabled=not numeric_headers, key="box_y_cols_v2_15")
                    ...
                    st.session_state.x_axis = st.selectbox("X축 범주 열 (선택)", box_x_options, index=box_x_options.index(current_box_x), 
                                                           disabled=not box_x_options[1:], key="box_x_col_v2_15")
                    ...
                    st.session_state.group_by_col = st.selectbox("색상 구분 열 (선택)", box_color_options, index=box_color_options.index(current_box_color), 
                                                                 disabled=not box_color_options[1:], key="box_color_col_v2_15")
                    st.session_state.box_points = st.selectbox("표시할 포인트", ["outliers", "all", "suspectedoutliers", False], index=["outliers", "all", "suspectedoutliers", False].index(st.session_state.box_points), key="box_points_v2_15")
                    ...
                    st.session_state.density_value_cols = st.multiselect("값 열 선택 (하나 이상, 숫자형)", numeric_headers, default=default_density_val, disabled=not numeric_headers, key="density_value_cols_v2_15")
                    ...
                    st.session_state.density_color_col = st.selectbox("색상 구분 열 (선택)", density_color_options, index=density_color_options.index(current_density_color), 
                                                                     disabled=not density_color_options[1:], key="density_color_col_v2_15")
                ...
                st.session_state.scatter_x_col = st.selectbox("X축 (숫자형)", numeric_headers if numeric_headers else ["선택 불가"], 
                                                              ...
                                                              disabled=not numeric_headers, key="scatter_x_v2_15")
                ...
                st.session_state.scatter_y_col = st.selectbox("Y축 (숫자형)", numeric_headers if numeric_headers else ["선택 불가"], 
                                                              ...
                                                              disabled=not numeric_headers, key="scatter_y_v2_15")
                ...
                st.session_state.scatter_color_col = st.selectbox("색상 구분 열 (선택)", color_options, index=color_options.index(current_scatter_color), 
                                                                 disabled=not color_options[1:], key="scatter_color_v2_15")
                ...
                if chart_type == '버블 차트 (Bubble Chart)':
                    ...
                    st.session_state.scatter_size_col = st.selectbox("버블 크기 열 (선택, 숫자형)", size_options, index=size_options.index(current_scatter_size), 
                                                                     disabled=not size_options[1:], key="scatter_size_v2_15")
                ...
                st.session_state.scatter_hover_name_col = st.selectbox("마우스 오버 시 이름 표시 열 (선택)", hover_name_options, index=hover_name_options.index(current_scatter_hover), 
                                                                       disabled=not hover_name_options[1:], key="scatter_hover_v2_15")
                ...
                st.session_state.radar_category_col = st.selectbox("범주/그룹 열 (Theta 그룹)", headers if headers else ["선택 불가"], 
                                                                   ...
                                                                   disabled=not headers, key="radar_cat_v2_15") 
                ...
                st.session_state.radar_value_cols = st.multiselect("값 열 선택 (Spokes - 여러개, 숫자형)", available_radar_values, default=default_radar_vals, 
                                                                  disabled=not available_radar_values, key="radar_val_v2_15")
                ...
                st.session_state.heatmap_corr_cols = st.multiselect("상관관계 분석 대상 열 (여러개, 숫자형)", numeric_headers, default=default_heatmap_cols, 
                                                                    disabled=len(numeric_headers)<2, key="heatmap_cols_v2_15")
                st.session_state.agg_method = 'Sum'; st.session_state.group_by_col = "None"; st.session_state.y_axis_secondary = "None"; st.session_state.x_axis = None
                ...
                st.session_state.pie_name_col = st.selectbox("레이블(이름) 열", headers if headers else ["선택 불가"], 
                                                             ...
                                                             disabled=not headers, key="pie_name_select_v2_15") 
                ...
                st.session_state.pie_value_col = st.selectbox("값 열", available_pie_values if available_pie_values else ["선택 불가"], 
                                                               ...
                                                               disabled=not available_pie_values, key="pie_value_select_v2_15")
        ...
        with st.expander("결측치 처리", expanded=False):
            st.session_state.mv_settings['selected_cols'] = st.multiselect("대상 열 선택 (결측치)", options=headers if headers else [], default=[col for col in st.session_state.mv_settings['selected_cols'] if col in (headers if headers else [])], disabled=not headers, key="mv_target_cols_v2_15")
            mv_method_options = ["결측치가 있는 행 전체 제거", "평균값으로 대체 (숫자형 전용)", "중앙값으로 대체 (숫자형 전용)", "최빈값으로 대체", "특정 값으로 채우기"]
            st.session_state.mv_settings['method'] = st.selectbox("처리 방법", options=mv_method_options, index=mv_method_options.index(st.session_state.mv_settings['method']), key="mv_method_v2_15")
            if st.session_state.mv_settings['method'] == "특정 값으로 채우기": 
                st.session_state.mv_settings['specific_value'] = st.text_input("채울 특정 값", value=st.session_state.mv_settings['specific_value'], key="mv_specific_val_v2_15")
            if st.button("결측치 처리 적용", key="apply_mv_button_v2_15"): 
                st.success("결측치 처리 설정이 저장되었습니다.") 
                apply_all_processing_steps() 
                st.rerun()

        with st.expander("이상치 처리", expanded=False):
            # (UI 업데이트 - st.session_state.ot_settings 사용)
            st.session_state.ot_settings['selected_cols'] = st.multiselect("대상 열 선택 (이상치)", options=numeric_headers if numeric_headers else [], default=[col for col in st.session_state.ot_settings['selected_cols'] if col in (numeric_headers if numeric_headers else [])], disabled=not numeric_headers, key="ot_target_cols_v2_15")
            ot_detection_options = ["IQR 방식", "Z-점수 방식"]
            st.session_state.ot_settings['detection_method'] = st.selectbox("탐지 방법", ot_detection_options, index=ot_detection_options.index(st.session_state.ot_settings['detection_method']), key="ot_detection_v2_15")
            if st.session_state.ot_settings['detection_method'] == "IQR 방식":
                st.session_state.ot_settings['iqr_multiplier'] = st.number_input("IQR 배수", min_value=0.1, value=st.session_state.ot_settings['iqr_multiplier'], step=0.1, key="ot_iqr_v2_15")
            else: # Z-점수 방식
                st.session_state.ot_settings['zscore_threshold'] = st.number_input("Z-점수 임계값", min_value=0.1, value=st.session_state.ot_settings['zscore_threshold'], step=0.1, key="ot_zscore_v2_15")
            ot_treatment_options = ["결측치로 처리 (NaN으로 대체)", "제거 (행 삭제)", "상한/하한 값으로 대체 (Winsorizing)"] # Winsorizing은 추가 구현 필요
            st.session_state.ot_settings['treatment_method'] = st.selectbox("처리 방법", ot_treatment_options, index=ot_treatment_options.index(st.session_state.ot_settings['treatment_method']), key="ot_treatment_v2_15")

            if st.button("이상치 처리 적용", key="apply_ot_button_v2_15"): 
                st.success("이상치 처리 설정이 저장되었습니다.")
                apply_all_processing_steps()
                st.rerun()

        with st.expander("중복 데이터 처리", expanded=False):
            # (UI 업데이트 - st.session_state.dd_settings 사용)
            st.session_state.dd_settings['subset_cols'] = st.multiselect("중복 검사 기준 열 (선택 안하면 모든 열 기준)", options=headers if headers else [], default=[col for col in st.session_state.dd_settings['subset_cols'] if col in (headers if headers else [])], disabled=not headers, key="dd_subset_cols_v2_15")
            dd_keep_options = ["첫 번째 행 유지", "마지막 행 유지", "모든 중복 행 제거"]
            st.session_state.dd_settings['keep_method'] = st.selectbox("유지할 행", dd_keep_options, index=dd_keep_options.index(st.session_state.dd_settings['keep_method']), key="dd_keep_v2_15")
            if st.button("중복 데이터 처리 적용", key="apply_dd_button_v2_15"): 
        ...
        with st.expander("필터링", expanded=False):
            # (UI는 기존과 동일, 단 filter_rules를 st.session_state에서 직접 관리)
            if st.button("필터 적용", key="apply_filters_v2_15"): 
                st.success("필터링 설정이 저장되었습니다.")
                apply_all_processing_steps()
                st.rerun()

        with st.expander("정렬", expanded=False):
            # (UI는 기존과 동일, 단 sort_rules를 st.session_state에서 직접 관리)
            if st.button("정렬 적용", key="apply_sorts_v2_15"): 
        ...
        with st.expander("피벗팅 (Pivoting)", expanded=False):
            # (UI 업데이트 - st.session_state.pivot_settings 사용)
            st.session_state.pivot_settings['index_cols'] = st.multiselect("인덱스 열", options=headers if headers else [], default=[col for col in st.session_state.pivot_settings['index_cols'] if col in (headers if headers else [])], disabled=not headers, key="pivot_index_v2_15")
            st.session_state.pivot_settings['columns_col'] = st.selectbox("컬럼으로 사용할 열", options=["선택 안함"] + (headers if headers else []), 
                                                                        index=(["선택 안함"] + headers).index(st.session_state.pivot_settings['columns_col']) if st.session_state.pivot_settings['columns_col'] in (["선택 안함"] + headers) and headers else 0, 
                                                                        disabled=not headers, key="pivot_columns_v2_15")
            if st.session_state.pivot_settings['columns_col'] == "선택 안함": st.session_state.pivot_settings['columns_col'] = None

            st.session_state.pivot_settings['values_cols'] = st.multiselect("값으로 사용할 열", options=numeric_headers if numeric_headers else [], default=[col for col in st.session_state.pivot_settings['values_cols'] if col in (numeric_headers if numeric_headers else [])], disabled=not numeric_headers, key="pivot_values_v2_15")
            st.session_state.pivot_settings['agg_func'] = st.selectbox("집계 함수", ['mean', 'sum', 'count', 'min', 'max'], index=['mean', 'sum', 'count', 'min', 'max'].index(st.session_state.pivot_settings['agg_func']), key="pivot_agg_v2_15")

            if st.button("피벗 적용", key="apply_pivot_v2_15"): 
                st.success("피벗팅 설정이 저장되었습니다.")
                apply_all_processing_steps()
                st.rerun()

        with st.expander("언피벗팅 (Unpivoting / Melt)", expanded=False):
            # (UI 업데이트 - st.session_state.unpivot_settings 사용)
            st.session_state.unpivot_settings['id_vars'] = st.multiselect("고정할 열 (ID 변수)", options=headers if headers else [], default=[col for col in st.session_state.unpivot_settings['id_vars'] if col in (headers if headers else [])], disabled=not headers, key="unpivot_id_v2_15")
            st.session_state.unpivot_settings['value_vars'] = st.multiselect("행으로 변환할 열 (선택 안하면 ID 제외 모든 열)", options=headers if headers else [], default=[col for col in st.session_state.unpivot_settings['value_vars'] if col in (headers if headers else [])], disabled=not headers, key="unpivot_value_v2_15")
            st.session_state.unpivot_settings['var_name'] = st.text_input("변수 이름 열의 새 이름", value=st.session_state.unpivot_settings['var_name'], key="unpivot_var_name_v2_15")
            st.session_state.unpivot_settings['value_name'] = st.text_input("값 열의 새 이름", value=st.session_state.unpivot_settings['value_name'], key="unpivot_value_name_v2_15")

            if st.button("언피벗 적용", key="apply_unpivot_v2_15"): 
        ...
        with st.expander("수식 기반 파생 변수 (간단)", expanded=False):
            st.session_state.derived_var_name = st.text_input("새 변수 이름", value=st.session_state.derived_var_name, key="derived_var_name_v2_15")
            st.session_state.derived_var_formula = st.text_area("수식 입력", value=st.session_state.derived_var_formula, height=100, key="derived_var_formula_v2_15", placeholder="예: (열1 + 열2) / 열3")
            if st.button("파생 변수 생성 적용", key="apply_derived_var_v2_15"): 
                if st.session_state.derived_var_name.strip() and st.session_state.derived_var_formula.strip():
        ...
        with st.expander("고급 파생 변수 편집기 (GUI)", expanded=st.session_state.show_adv_derived_var_builder):
            st.write("GUI를 사용하여 조건부 규칙, 창 함수 또는 값 매핑 기반의 파생 변수를 생성 및 관리합니다.")
            if st.button("➕ 새 고급 파생 변수 만들기", key="add_new_adv_derived_var_btn"):
                ...
                st.session_state.adv_builder_var_type = 'conditional' # 기본 유형
                ...
                st.session_state.adv_builder_mapping_source_col = first_col_for_builder
                st.session_state.adv_builder_mapping_rules = [{'id': str(uuid.uuid4()), 'values_to_map': '', 'new_group_name': ''}]
                st.session_state.adv_builder_mapping_default_group = ''
                if st.session_state.editing_adv_derived_var_name: 
                    for flag_key_suffix in ["rules_loaded_for_", "win_loaded_for_", "map_loaded_for_"]:
            ...
            if st.session_state.show_adv_derived_var_builder:
                ...
                st.session_state.adv_builder_var_type = st.selectbox("유형:", ADV_DERIVED_VAR_TYPES,
                                                                    index=ADV_DERIVED_VAR_TYPES.index(var_type_default) if var_type_default in ADV_DERIVED_VAR_TYPES else 0,
                ...
                elif st.session_state.adv_builder_var_type == 'mapping': # 조건부 그룹핑 UI
                    if is_editing_adv and current_adv_def.get('type') == 'mapping':
                        if not st.session_state.get(f"adv_map_loaded_for_{st.session_state.editing_adv_derived_var_name}", False):
                            map_conf = current_adv_def.get('config', {})
                            st.session_state.adv_builder_mapping_source_col = map_conf.get('source_col', (available_vars_for_adv[0] if available_vars_for_adv else ''))
                            st.session_state.adv_builder_mapping_rules = [dict(r, id=r.get('id', str(uuid.uuid4()))) for r in map_conf.get('rules', [])]
                            st.session_state.adv_builder_mapping_default_group = map_conf.get('default_group', '')
                            st.session_state[f"adv_map_loaded_for_{st.session_state.editing_adv_derived_var_name}"] = True
                    elif not is_editing_adv and not st.session_state.adv_builder_mapping_rules: # 새 변수이고 규칙 없으면 기본 추가
                        st.session_state.adv_builder_mapping_source_col = available_vars_for_adv[0] if available_vars_for_adv else ''
                        st.session_state.adv_builder_mapping_rules = [{'id': str(uuid.uuid4()), 'values_to_map': '', 'new_group_name': ''}]
                        st.session_state.adv_builder_mapping_default_group = ''


                    st.session_state.adv_builder_mapping_source_col = st.selectbox("매핑할 원본 열:", available_vars_for_adv if available_vars_for_adv else ["선택 불가"], 
                                                                                    index=available_vars_for_adv.index(st.session_state.adv_builder_mapping_source_col) if st.session_state.adv_builder_mapping_source_col in available_vars_for_adv and available_vars_for_adv else 0, 
                                                                                    disabled=not available_vars_for_adv, key="adv_map_source_col_select")
                    st.markdown("--- **매핑 규칙** ---")
                    num_map_rules = len(st.session_state.adv_builder_mapping_rules)
                    for i in range(num_map_rules):
                        map_rule = st.session_state.adv_builder_mapping_rules[i]
                        if 'id' not in map_rule: map_rule['id'] = str(uuid.uuid4())
                        cols_map_rule = st.columns([4, 4, 1])
                        map_rule['values_to_map'] = cols_map_rule[0].text_input(f"매핑할 값(들) (쉼표로 구분)", value=map_rule.get('values_to_map', ''), key=f"adv_map_values_{map_rule['id']}")
                        map_rule['new_group_name'] = cols_map_rule[1].text_input(f"새 그룹명", value=map_rule.get('new_group_name', ''), key=f"adv_map_group_name_{map_rule['id']}")
                        if num_map_rules > 1 and cols_map_rule[2].button("➖", key=f"adv_remove_map_rule_{map_rule['id']}", help="이 매핑 규칙 삭제"):
                            st.session_state.adv_builder_mapping_rules.pop(i)
                            st.rerun()
                    
                    if st.button("➕ 매핑 규칙 추가", key="adv_add_map_rule_btn"):
                        st.session_state.adv_builder_mapping_rules.append({'id': str(uuid.uuid4()), 'values_to_map': '', 'new_group_name': ''})
                        st.rerun()
                    
                    st.session_state.adv_builder_mapping_default_group = st.text_input("기본 그룹명 (매핑되지 않은 값들 대상, 비워두면 원본 값 유지)", 
                                                                                        value=st.session_state.adv_builder_mapping_default_group, 
                                                                                        key="adv_map_default_group_input")
                ...
                if adv_btn_cols[0].button("💾 고급 파생 변수 저장", type="primary", use_container_width=True, key="save_adv_derived_var_btn"):
                    ...
                    if not new_adv_var_name_val: st.error("파생 변수 이름을 입력해야 합니다.")
                        ...
                        elif st.session_state.adv_builder_var_type == 'mapping':
                            if not st.session_state.adv_builder_mapping_source_col: st.error("매핑할 원본 열을 선택해야 합니다."); valid_def = False
                            if not st.session_state.adv_builder_mapping_rules and valid_def: st.error("최소 하나 이상의 매핑 규칙이 필요합니다."); valid_def = False
                            for r_idx, r_map in enumerate(st.session_state.adv_builder_mapping_rules):
                                if not r_map.get('values_to_map','').strip() or not r_map.get('new_group_name','').strip():
                                    st.error(f"매핑 규칙 {r_idx+1}: '매핑할 값'과 '새 그룹명'은 모두 필수입니다."); valid_def = False; break
                            if valid_def:
                                adv_definition_to_save['config'] = {
                                    'source_col': st.session_state.adv_builder_mapping_source_col,
                                    'rules': st.session_state.adv_builder_mapping_rules,
                                    'default_group': st.session_state.adv_builder_mapping_default_group.strip() # 공백 제거
                                }
...
if df is None:
    ...
    if not df.empty:
        ...
        st.info("현재 데이터프레임이 비어 있습니다.")

    st.subheader("시각화 결과")
    chart_placeholder = st.empty()
    if df.empty:
        chart_placeholder.info("차트를 생성할 데이터가 없습니다.")
    else:
        try:
            fig = None
            # ... (기존 차트 생성 로직 - 변경 없음, 단 df가 비어있는 경우를 고려하여 Plotly 함수 호출 전에 확인 필요) ...
            # (이 부분은 매우 길어서 생략합니다. 제공해주신 코드의 해당 부분을 그대로 사용하시되,
            #  processed_df_for_chart.empty 와 같은 조건을 추가하여 Plotly 함수 호출을 방어하십시오.)
            # --- 차트 생성 로직 시작 (기존 코드에서 가져옴, df.empty 방어 코드 추가 필요) ---
            chart_type = st.session_state.chart_type
            ...
            if processed_df_for_chart.empty: # 차트 생성 전 데이터프레임 비었는지 확인
                chart_placeholder.info("차트를 생성할 데이터가 없습니다 (처리 결과가 비어있을 수 있습니다).")
                st.stop()
            ...
            if chart_type == '파이 (Pie)':
                if pie_name_col in processed_df_for_chart and pie_value_col in processed_df_for_chart:
                    pie_data_agg = processed_df_for_chart.groupby(pie_name_col, as_index=False).agg({pie_value_col: current_agg_func})
                    if not pie_data_agg.empty: fig = px.pie(pie_data_agg, names=pie_name_col, values=pie_value_col, title=f"{pie_name_col} 별 {pie_value_col} 분포 ({agg_method} 기준)")
                    else: chart_placeholder.info("파이 차트: 집계 후 데이터가 없습니다.")
                else: chart_placeholder.warning("파이 차트: 선택된 열이 데이터에 없습니다.")
            elif chart_type == '히스토그램 (Histogram)':
                y_val_for_hist = y_multiple[0] if y_multiple and y_multiple[0] in processed_df_for_chart else None
                if not y_val_for_hist : chart_placeholder.warning("히스토그램에 사용할 유효한 값 열을 선택해주세요."); st.stop()
                if len(y_multiple) > 1: st.info(f"히스토그램은 현재 하나의 값 열('{y_val_for_hist}')에 대해서만 그려집니다. 색상 구분 열을 활용하세요.")
                fig = px.histogram(processed_df_for_chart, x=y_val_for_hist, color=group_by_col if group_by_col != "None" and group_by_col in processed_df_for_chart else None, nbins=st.session_state.hist_bins, title=f"{y_val_for_hist}의 분포" + (f" (색상: {group_by_col})" if group_by_col != "None" and group_by_col in processed_df_for_chart else ""))
            elif chart_type == '박스 플롯 (Box Plot)':
                ...
                if not y_multiple or not all(y_col in processed_df_for_chart.columns for y_col in y_multiple): chart_placeholder.warning("박스 플롯에 사용할 유효한 Y축 값 열을 선택해주세요."); st.stop()
                ...
                if not density_values or not all(dv in processed_df_for_chart.columns for dv in density_values): chart_placeholder.warning("밀도 플롯에 사용할 유효한 값 열을 선택해주세요."); st.stop()
                current_density_color = density_color if density_color and density_color in processed_df_for_chart.columns else None
                if len(density_values) == 1 and not current_density_color:
                ...
                elif len(density_values) == 1 and current_density_color:
                     fig = px.histogram(processed_df_for_chart, x=density_values[0], color=current_density_color, marginal="rug", histnorm='probability density', barmode="overlay", opacity=0.7, title=f"{density_values[0]}의 밀도 플롯 (색상: {current_density_color})")
                ...
                if not radar_vals or not all(rv in df_for_radar.columns for rv in radar_vals): chart_placeholder.warning("레이더 차트에 사용할 유효한 값 열을 선택해주세요."); st.stop()
                ...
                if not heatmap_cols or len(heatmap_cols) < 2 or not all(hc in processed_df_for_chart.columns for hc in heatmap_cols) : chart_placeholder.warning("히트맵에 사용할 유효한 숫자형 열을 2개 이상 선택해주세요."); st.stop()
                ...
                if scatter_x not in processed_df_for_chart or scatter_y not in processed_df_for_chart: chart_placeholder.warning("분산형 차트: X축 또는 Y축 열이 데이터에 없습니다."); st.stop()
                current_scatter_color = scatter_color if scatter_color != "None" and scatter_color in processed_df_for_chart else None
                current_scatter_hover = scatter_hover if scatter_hover != "None" and scatter_hover in processed_df_for_chart else None
                fig = px.scatter(processed_df_for_chart, x=scatter_x, y=scatter_y, color=current_scatter_color, hover_name=current_scatter_hover, title=f"{scatter_x} vs {scatter_y} 관계")
            
            elif chart_type == '버블 차트 (Bubble Chart)':
                if scatter_x not in processed_df_for_chart or scatter_y not in processed_df_for_chart: chart_placeholder.warning("버블 차트: X축 또는 Y축 열이 데이터에 없습니다."); st.stop()
                current_scatter_color = scatter_color if scatter_color != "None" and scatter_color in processed_df_for_chart else None
                current_scatter_size = scatter_size if scatter_size != "None" and scatter_size in processed_df_for_chart else None
                current_scatter_hover = scatter_hover if scatter_hover != "None" and scatter_hover in processed_df_for_chart else None
                fig = px.scatter(processed_df_for_chart, x=scatter_x, y=scatter_y, color=current_scatter_color, size=current_scatter_size, hover_name=current_scatter_hover, title=f"{scatter_x} vs {scatter_y} (크기: {scatter_size}, 색상: {scatter_color})")
                ...
                if y_cols_to_aggregate and grouping_cols:
                    try:
                        # 집계 전, y_cols_to_aggregate의 모든 컬럼이 processed_df_for_chart에 있는지 확인
                        if not all(col in processed_df_for_chart.columns for col in y_cols_to_aggregate):
                            missing_agg_cols = [col for col in y_cols_to_aggregate if col not in processed_df_for_chart.columns]
                            chart_placeholder.error(f"집계 대상 열 {missing_agg_cols}이(가) 현재 데이터에 없습니다."); st.stop()
                ...
                if y_secondary != "None" and y_secondary in numeric_headers and not is_stacked_chart:
                    ...
                    current_color_col_for_chart = color_col_for_chart if color_col_for_chart and color_col_for_chart in processed_df_for_chart.columns else None
                    if chart_type == '막대 (Bar)': fig = px.bar(processed_df_for_chart, x=x_axis, y=y_plot_val, color=current_color_col_for_chart, barmode='group' if current_color_col_for_chart else 'relative', title=f"{x_axis} 별 Y값 ({agg_method})")
                    elif chart_type == '누적 막대 (Stacked Bar)':
                        if current_color_col_for_chart: fig = px.bar(processed_df_for_chart, x=x_axis, y=y_plot_val, color=current_color_col_for_chart, barmode='stack', title=f"{x_axis} 별 {current_color_col_for_chart} 그룹 {y_plot_val} (누적, {agg_method})")
                        else: 
                    ...
                    elif chart_type == '선 (Line)': fig = px.line(processed_df_for_chart, x=x_axis, y=y_plot_val, color=current_color_col_for_chart, markers=True, title=f"{x_axis} 별 Y값 ({agg_method})")
                    elif chart_type == '누적 영역 (Stacked Area)':
                        if current_color_col_for_chart: fig = px.area(processed_df_for_chart, x=x_axis, y=y_plot_val, color=current_color_col_for_chart, title=f"{x_axis} 별 {current_color_col_for_chart} 그룹 {y_plot_val} (누적 영역, {agg_method})")
                        else:
